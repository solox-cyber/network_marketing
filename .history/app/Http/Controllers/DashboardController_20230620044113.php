...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::id()) {
            ...
            if ($usertype == 'user') {
                ...
                if ($totalCount >= ($digit * 4 + 1)) {
                    $level = 1;
                    $commissionPercentage = 10;
                }elseif ($totalCount >= ($digit * 16 + 5)) {
                    $level = 2;
                    $commissionPercentage = 8;
                }elseif ($totalCount >= ($digit * 64 + 21)) {
                    $level = 3;
                ...
                return view('dashboard', compact('course_name','commissionPercentage', 'leverp', 'about_course', 'WalletAmountGross', 'contactCount','digit', 'course', 'userCount', 'totalCount', 'WalletAmount', 'commissionPercentage', 'level', 'service', 'serial_number'));
    ...
    public function courseShow($id)
    {
        if (auth()->user()->usertype === 'admin') {
            ...
            return view('admin.course-list', compact('salesRep', 'salesReps', 'numberOfContacts', 'salescontacts', 'contacts'));
        }
    }
    ...
    public function searchSales(Request $request)
    {
        ...
        $salesReps = SalesRep::where(function ($query) use ($search) {
            $query->where('name', 'like', "%$search%")
                ->orWhere('email', 'like', "%$search%");
        })->get();
