...
class DashboardController extends Controller
{
    ...
    public function resetcreate(Request $request, $token = null)
    {
        $user = Auth::user();

        return view('resetpass')->with(
            ['token' => $token, 'email' => $Auth->email]
        );
    }
    public function resetstore(Request $request)
    {
        ...
        if ($resetPasswordStatus == Password::PASSWORD_RESET) {
            return redirect($this->redirectPath())->with('status', trans($resetPasswordStatus));
        } else {
            return back()->withErrors(['email' => trans($resetPasswordStatus)]);
        }
    }
