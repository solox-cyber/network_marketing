...
class DashboardController extends Controller
{
    ...
    public function commission($id)
    {
        ...
        if (auth()->user()->usertype === 'admin' || auth()->user()->usertype === 'subadmin') {
            ...
            $walletSatements = WalletStatement::with('user')->paginate(10);

        foreach ($walletSatements as $walletSatement) {
            // Get the user's wallet amount
            $userWalletAmount = User::where('id', $walletSatement->user_id)->value('walletamount');
            $walletSatement->user->walletamount = $userWalletAmount;
        }

            $walletStatement = WalletStatement::find($id);
            $user = $walletStatement->user;
            return view('admin.commission', ['walletStatement' => $walletStatement, 'user' => $user], compact('WalletAmount', 'WalletAmountGross','userWalletAmount'));
        }
