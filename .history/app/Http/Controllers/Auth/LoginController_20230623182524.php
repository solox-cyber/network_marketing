    ...
    public function store(Request $request)
{
    ...
    // If the user is not found in the User model, check the SalesRep model
    if (!$user) {
        $salesRep = SalesRep::where('email', $credentials['email'])->first();

        if ($salesRep && Auth::attempt($credentials, $request->filled('remember'))) {
            return redirect()->intended(RouteServiceProvider::HOME);
        }
    }

    // Check if the user account is deactivated
    if (!$user || !$user->isActive() || !Auth::attempt($credentials, $request->filled('remember'))) {
