...
class DashboardController extends Controller
{
    ...
    public function assignContacts(Request $request)
    {
        ...
        // Check if the $contactIds array is empty or null
        if (empty($contactIds)) {
            // Handle the case when no contacts are selected
            return redirect()->back()->with('error', 'No contacts selected.');
        }

        // Convert $contactIds to an array
        $contactIds = array_values($contactIds);

        // Get the selected sales rep ID
        $salesRepId = $request->input('sales_rep_id');

        // Assign the selected contacts to the sales rep
