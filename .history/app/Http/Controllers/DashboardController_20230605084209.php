...
class DashboardController extends Controller
{
    ...
    if (auth()->user()->usertype === 'admin') {
        ...
        // Assuming $contact->created_at is a valid DateTime instance or string representation
         // Format the created_at value for each contact
         $formattedContacts = $contacts->map(function ($contact) {
            $contact->formattedCreatedAt = Carbon::parse($contact->created_at)->format('d M Y, h:i a');
            return $contact;
        });

        return view('admin.users-list',compact('contacts,formattedContacts'));
    }
