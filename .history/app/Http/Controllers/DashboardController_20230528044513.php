...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $WalletAmount = number_format($pay * ($commissionPercentage / 100), 2, '.', ',');

        $user->level = $level;
    $user->wallet_amount = $WalletAmount;
    $user->commission_status = 'pending';
    $user->commission_percentage = WalletAmount;
    $user->save();

        return view('dashboard', compact('contactCount', 'userCount', 'totalCount','WalletAmount','commissionPercentage','level'));
    }
