...
use Controllers\Illuminate\Support\Facades\Mail
...
class DashboardController extends Controller
{
    ...
    Mail::to($contact->email)->send(new ContactMail($user, $contactId, $salesId));
...
public function sendcontactsemail($sales_id, $contact_id)
{
    try {
        ...
        // Get the user object based on the authenticated user (assuming it's the sales representative)
        $user = User::findOrFail($sales_id);

        // Send the email to the user's email address with the contact ID and sales ID
        Mail::to($contact->email)->send(new InvitesMail($user, $contact_id, $sales_id));
