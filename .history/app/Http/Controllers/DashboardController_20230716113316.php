...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::id()) {
            ...
            if ($usertype == 'user') {
                ...
                if ($totalCount == 0 && $totalCount < ($digit * 4 + 1)) {
                    $level = 0;
                    $commissionPercentage = 0;
                    $WalletAmountGross = $pay * ($commissionPercentage / 100);
                } elseif ($totalCount >= ($digit * 4 + 1) && $totalCount < ($digit * 16 + 5)) {
                    $level = 1;
                    $commissionPercentage = 10;
                    $WalletAmountGross = ($pay * ($commissionPercentage / 100)) * 4;
                } elseif ($totalCount >= ($digit * 16 + 5) && $totalCount < ($digit * 64 + 21)) {
                    $level = 2;
                    $commissionPercentage = 8;
                    $WalletAmountGross = ($pay * ($commissionPercentage / 100)) * 16;
                } elseif ($totalCount >= ($digit * 64 + 21) && $totalCount < ($digit * 256 + 85)) {
                    $level = 3;
                    $commissionPercentage = 6;
                    $WalletAmountGross = ($pay * ($commissionPercentage / 100)) * 64;
                } elseif ($totalCount >= ($digit * 256 + 85)) {
                    $level = 4;
                ...
                return view('dashboard', compact('courseDetails', 'totalCount', 'digit', 'contactCount', 'totalCount', 'course', 'userCount', 'totalCount', 'WalletAmount', 'commissionPercentage', 'level', 'service', 'serial_number', 'enrollment_status'));
    ...
    public function salescommissions()
    {
        if (auth()->user()->usertype === 'admin' || auth()->user()->usertype === 'subadmin') {
            ...
            return view('admin.salescommission-list', ['walletStatements' => $walletStatements], compact('WalletAmount', 'WalletAmountGross'));
        }
    ...
    public function convert(Request $request)
    {
        ...
        $user->password = bcrypt($password); // Generate an automatic password
        $user->phone_number = $contact->phone;
        $user->sales_rep_id = $request->input('salesrep');
        $user->upline = $contact->user_id;
        ...
        // Save any additional details to the contacts table

        // Send email verification link and newly generated password to the provided email
        // Mail::to($validatedData['email'])->send(new EmailVerification($user, $serialNumber));
        ...
        if ($user instanceof MustVerifyEmail && !$user->hasVerifiedEmail()) {
            ...
            $user->sendEmailVerificationNotifications($password);
        }
        ...
        $contacts = Contact::with('user', 'salesRep')->paginate(10);
        // Redirect the user to a success page or return a success message
        return view('salesrep.invitation', compact('salesReps', 'contacts'))->with('success', 'User details saved successfully.');
    }
