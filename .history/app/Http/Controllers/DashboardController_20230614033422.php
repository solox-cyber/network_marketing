...
class DashboardController extends Controller
{
    ...
    public function userlist()
    {
        ...
        if (auth()->user()->usertype === 'admin') {
            ...
            // Get the unique course IDs from the users table
        $courseIds = $contacts->pluck('course_id')->unique()->toArray();

        // Get the courses with the selected course IDs
        $courses = Course::whereIn('id', $courseIds)->get();

            return view('admin.users-list', compact('contacts','courses'));
        }
