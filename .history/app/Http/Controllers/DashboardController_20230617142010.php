...
class DashboardController extends Controller
{
    ...
    public function searchSales($id)
{
    if (auth()->user()->usertype === 'admin') {
        $search = request()->input('search');

        $salesReps = SalesRep::findOrFail($id);

        $contacts = $salesReps->contacts()->where(function ($query) use ($search) {
            $query->where('name', 'like', "%$search%")
                ->orWhere('email', 'like', "%$search%");
        })->get();

        return view('admin.salessearch-results', compact('contacts', 'salesReps'));
    }
}
