...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::id()) {
            ...
            if ($usertype == 'user') {
                ...
                $totalContacts = Contact::count();
                $totalSalesReps = SalesRep::count();
                return view('salesdash', compact('totalUsers', 'totalContacts', 'totalSalesReps'));
            } else {
    ...
    public function convert(convert(Request $request)
    {
        $validatedData = $request->validate([
            'email' => 'required|email',
            'service' => 'required',
        ...
        $user->password = bcrypt(Str::random(10)); // Generate an automatic password
        $user->phone_number = $contact->phone_number;
        $user->country = $contact->country;
        $user->sales_rep_id = Auth::id();
        ...
        Mail::to($validatedData['email'])->send(new EmailVerification($user));

        // Redirect the user to a success page or return a success message
        return redirect()->back()->with('success', 'User details saved successfully. Please check your email for verification instructions.');
    }
