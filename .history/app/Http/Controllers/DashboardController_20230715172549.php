    ...
    public function index()
{
    ...
    if ($usertype == 'user') {
        $courseDetails = Course::where('id', $user->service)->get();

        $contactCount = $user->contacts()->count();

        $userCount = User::where('usertype', 'user')->count();

        $totalCount = User::where('usertype', 'user')->where('id', '>', $user->id)->count();
        ...
        $Rate = 1 / 777.58;
        $pay = ($countryCode === 'NG') ? 100000 : 100000 * $Rate;
        ...
        $WalletAmount = ($paymentStatus > 0) ? $WalletAmountGross - (($countryCode === 'NG') ? $paymentStatus : $paymentStatus * $Rate) : $WalletAmountGross;
        ...
        return view('dashboard', compact('courseDetails', 'contactCount', 'totalCount', 'userCount', 'WalletAmount', 'commissionPercentage', 'level', 'service', 'serial_number', 'enrollment_status'));
    }
