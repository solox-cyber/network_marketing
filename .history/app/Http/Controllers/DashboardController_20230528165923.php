...
class DashboardController extends Controller
{
    ...
    $totalWithdrawRequests = WalletStatement::where('user_id', $user->id)where('type', 'withdrawal')->count();

    $paymentStatus = WalletStatement::where('user_id', $user->id)
    ->where('payment_status', 'verified')
    ->sum('amount');
    // Check if the payment status is verified
    $paymentStatus = 'verified'; // Replace this with your actual payment status variable
    if ($paymentStatus === 'verified') {
        // Withdraw from $walletAmount
        $WalletAmount =  $WalletAmountGross - $totalWithdrawRequests;
    } else {
        // Payment status is not verified, do not withdraw
