    ...
    public function index()
    {
        ...
        if (Auth::id()) {
            ...
            if ($usertype == 'user') {
                ...
                // Get the total number of contacts assigned to the user
                $totalContacts = $user->contacts()->count();

                // Get the total number of users with sales_id equal to the user's ID
                $totalUsers = User::where('sales_rep_id', $user->id)->count();
                return view('salesdash', compact('totalUsers', 'totalContacts'));
            } else {
