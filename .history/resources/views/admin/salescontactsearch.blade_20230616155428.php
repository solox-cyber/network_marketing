                    ...
                    <div class="d-flex align-items-center w-100">
                        <div class="ms-4 d-flex align-items-center flex-fill">
                            ...
                            <div class="fw-semibold fs-7 text-muted flex-shrink-0">
                                ...
                                <!-- salescontacts already assigned to a sales rep -->
                                <form method="POST" action="{{ route('sales-reps.salescontactss.assign', ['salesRepId' => $salesRep->id, 'salescontactsId' => $salescontacts->id]) }}">
                                    @csrf
                                    <input type="hidden" name="salescontacts_id" value="{{ $salescontacts->id }}">
                                    <button class="btn btn-sm btn-danger me-2">Reassign</button>
                                    @else
                                    <form method="POST" action="{{ route('sales-reps.salescontactss.assign', ['salesRepId' => $salesRep->id, 'contactId' => $contact->id]) }}">
                                        @csrf
