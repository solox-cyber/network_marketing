            ...
            <form class="form w-100" action="{{ route('register') }}" method="POST">
                ...
                <div class="fv-row mb-8">
                    <select name="service" aria-label="Select a Course" data-control="select2" data-placeholder="Select a course..." class="form-select form-select-solid form-select-lg fw-semibold">
                        ...
                        <option value="{{ $course->id }}" data-tuition="{{ $course->tuition_fee }}">{{ $course->course_name }}</option>
                        @endforeach
                    </select>
                </div>
                <!--begin::Input group-->
                ...
                <script>
                    ...
                    paymentButton.addEventListener('click', () => {
                        ...
                        const selectedOption = document.querySelector('select[name="service"] option:checked');
                         ...
                         const countryCode = '{{$countryCode}}'; // Replace with your actual code to get the country code

                         if (countryCode === 'NG') {
        tuitionFee = parseFloat(tuitionFee).toFixed(2);
    } else {
        const rate = 1 / 777.58;
        tuitionFee = (tuitionFee * rate).toFixed(2);
    }
